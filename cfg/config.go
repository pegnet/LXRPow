...
func (c *Config) Init() {
	...
	pBlockTime := flag.Uint64("blocktime", 600, "Block Time in seconds (600 would be 10 minutes)")
	...
	fmt.Printf("\nminer --index=%d --tokenurl=\"%s\" --instances=%d --loop=%d --bits=%d --phrase=\"%s\""+
		" --randomize=%v --difficulty=0x%x --diffwindow=%d --blocktime=%d --timed=%v\n\n",
		c.Index, c.TokenURL, c.Instances, c.Loop, c.Bits, c.Phrase,
